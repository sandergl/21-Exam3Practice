    ...
    for k in range(len(sequence_of_sequences)):
        for i in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][i]) is int:
                new.append(sequence_of_sequences[k][i])
    return new
    # ------------------------------------------------------------------
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'HELLO'
    answer = big_letters([(3, 1, 4), [], ['a', 'A'], 'Hey Ellis!', 'Learn to Live', 'O', 'abcdefgh'])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        for i in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k]) is str:
                if sequence_of_sequences[k][i].isupper() is True:
                    new = new + sequence_of_sequences[k][i]
    return new
    # ------------------------------------------------------------------
